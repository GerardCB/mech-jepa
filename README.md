...
At every step: `env.render() → VideoSAUR DINOv2 ViT → slot corrector → 4 slots → CEM planner → action → env.step()`

Three conditions: Expert (left), Frozen MechJEPA (centre), A-B-M with System M (right).

<!-- GIF will be generated by: python scripts/eval_live_pusht.py --episodes 3 -->
![Expert | Frozen | A-B-M](assets/eval_live/pusht_ep01.gif)
...
│   ├── eval_live_pusht.py     # Phase 3 closed-loop eval (real env renders)
│   ├── abm_pusht.py           # Phase 3 latent-only A-B-M benchmark
